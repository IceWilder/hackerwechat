...
define([
    ...
    var Confirm = function(msg,callback) {
        ...
        var html = '<div id="alertFram" style="position: fixed; left: 5%; top: 43%; width: 90%; margin-top: -75px; z-index: 10001; border-radius: 6px;">' +
            ...
            '<div class="nm-ok" style=" width: 50%;float: left;text-align: center;border-right:solid 1px #e4e4ee;color: dodgerblue;line-height: 39px">确定</div>' +
        ...
        confirm.one(".nm-cancel").on('click',function(){
            $("body").css("overflow","auto");
            confirm.remove();
        });
        confirm.one(".nm-ok").on('click',function(){
            $("body").css("overflow","auto");
            confirm.remove();
            if (typeof(callback) == 'function') {
                callback();
