    ...
    $(function () {
        ...
        $("#publish").click(function(){
            ...
            var course_bill=$(".bill img").attr("src");
              var course_type=$(":checked").attr("id");
            var course_intro=$("textarea").val();
            var params={course_type:course_type,
            ...
            if(!time_flag){
               ...
               // params.videos_num=videos_num;
                console.log(params);
                bootbox.confirm("确定增加该课程？",function(result){
                    if(result) {
                        IO.post("/hewuqi/api/addCourse",params,function(d){
                            bootbox.alert(d.msg,function(){
                                if (d.code == 0) {
                                    location.href = '/hewuqi/teacherinfo?user_id='+user_id;
                                }
